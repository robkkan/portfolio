window.addEventListener("load", () => {
  function initializeAnimation() {
              ...
              <div class="camera-shutter__lens">
                <div class="camera-shutter__triangles">
                  ${Array(6)
                    ...
                    .map(
                      (_, i) =>
                        `<div class="camera-shutter__triangle camera-shutter__triangle--${
                          i + 1
                        }"></div>`
                    )
                    .join("")}
    ...
    function animateShutter() {
      ...
      const shutterTriangles = document.querySelectorAll(
        ".camera-shutter__triangle"
      );
      ...
      const animations = [
        {
          className: "camera-shutter--f2",
          delay: 1000,
          transitionDuration: "1s",
        },
        {
          className: "camera-shutter--f8",
          delay: 1500,
          transitionDuration: "1s",
        },
        {
          className: "camera-shutter--f22",
          delay: 2250,
          transitionDuration: "1s",
        },
        {
          className: "camera-shutter--f45",
          delay: 3000,
          transitionDuration: "1s",
        },
      ];
      ...
      function runAnimation(index = 0) {
        if (index >= animations.length) {
          setTimeout(
            () => runAnimation(0),
            CAMERA_ANIMATION_INTERVAL -
              animations.reduce((sum, anim) => sum + anim.delay, 0)
          );
          return;
        }
